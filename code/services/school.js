...
async function create(body) {
  let user
  try {
    user = await prisma.School.create({
      data: body,
    });
  } catch (error) {
    throw error;
...
async function update(id, updatedData) {
  ...
  try {
    updatedPermission = await prisma.School.update({
      where: { id: id - 0 },
      data: updatedData,
    });
...
async function get_all(page = 1, pageSize = 10, filters = []) {
  ...
  const totalUsers = await prisma.School.count({
    where: where
  });
...
async function get_id(id) {
  const post = await prisma.School.findUnique({
